...
export default function AddTask() {
  ...
  const validateTitle = (title) => {
    ...
    const specialCharPattern = /[^a-zA-Z0-9\s]/; // Blocks all special characters
    const maxLength = 100;
    const minLength = 2;
  ...
  const handleTitleChange = (e) => {
    ...
    // Filter out all special characters immediately
    ...
    if (value !== filteredValue) {
      showWarningAlert('Invalid Characters Removed', 'Only letters, numbers, and spaces are allowed in task title');
    }
  ...
  const handleDescriptionChange = (e) => {
    ...
    // If special characters were removed, show warning
    if (value !== filteredValue) {
      showWarningAlert('Invalid Characters Removed', 'Only letters, numbers, and spaces are allowed in description');
    }
  ...
  const handleTitleKeyDown = (e) => {
    ...
    if (!allowedPattern.test(e.key) && e.key !== 'Backspace' && e.key !== 'Delete' && 
      ...
      // Show brief warning
      showWarningAlert('Character Blocked!', `Only letters, numbers, and spaces are allowed in task title`);
      return false;
    }
  };
  ...
  const handleDescriptionKeyDown = (e) => {
    ...
    if (!allowedPattern.test(e.key) && e.key !== 'Backspace' && e.key !== 'Delete' && 
      ...
      // Show brief warning
      showWarningAlert('Character Blocked!', `Only letters, numbers, and spaces are allowed in description`);
      return false;
    }
  };
  ...
  const handleTitlePaste = (e) => {
    ...
    if (pastedText !== filteredText) {
      showWarningAlert('Characters Filtered', 'Only letters, numbers, and spaces were kept from pasted text');
    }
    
    const newValue = newTask + filteredText;
    const truncatedValue = newValue.substring(0, 100); // Respect max length
  ...
  const handleDescriptionPaste = (e) => {
    ...
    const truncatedValue = newValue.substring(0, 500); // Respect max length
      ...
      <div className="pt-20 p-4">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <h2 className="text-3xl font-bold text-primary absolute left-1/2 transform -translate-x-1/2">Add New Task</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <motion.div 
              ...
              className="card p-6"
            >
              <h3 className="text-xl font-semibold text-primary mb-6">Task Details</h3>
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    className={`flex-1 py-2 text-sm font-medium rounded transition-all duration-200 ${
                      Object.values(errors).some(error => error) || !newTask.trim()
                        ? 'bg-gray-400 dark:bg-gray-600 text-gray-200 cursor-not-allowed'
                        : 'btn-primary hover:shadow-lg transform hover:scale-105'
                    }`}
                    onClick={createTask}
                    disabled={Object.values(errors).some(error => error) || !newTask.trim()}
                  >
                    {Object.values(errors).some(error => error) ? '⚠️ Fix Errors' : '✅ Create Task'}
                  </button>
                  <button 
                    onClick={goBack}
            ...
            {/* Right Column - Calendar and Tips */}
            <div className="space-y-6">
              {/* Due Date Section */}
              <motion.div 
                initial={{ opacity: 0, x: 20 }} 
              ...
              >
                ...
                <div className={`p-3 rounded-lg ${
                  ...
                  <p className={`text-sm font-medium ${
                    ...
                    Selected Date: {dueDate.toLocaleDateString('en-US', { 
                      weekday: 'long', 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
